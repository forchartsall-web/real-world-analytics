...
2.  Fit aggregation models : Linear problem is solved to find weights of predictor variables. lpSolve library used (source: AggWaFit718.R).
...
⦁ Low pvalue (e.g. \< 0.05): data deviate from normality.​ So if you apply the test to the transformed data x^n^ as shapiro.test(x^n^)\$p.value, a larger pvalue means that particular power n ,makes the distribution of x^n^ closer to normal (in the sense of this test).
