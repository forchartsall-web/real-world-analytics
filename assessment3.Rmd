...
```{r}
my.data <- the.data[sample(1:nrow(the.data),num_row), 1:7]
```
...
```
```{r}
colnames(my.data)<- c("X1","X2","X3","X4","X5","X6","Y")
```
...
```{r}
for (k in paste0("X", 1:6)) { hist(my.data[, k], main = k, xlab = k) }
```
```{r}
...
```{r}
P=0.25
data.transformed[,2]=(data.transformed[,2])^P  
```
```{r}
p=10
data.transformed[,3]=variables_to_transform[,3]
data.transformed[,3]=unit.z(data.transformed[,3])
data.transformed[,3]=(data.transformed[,3])^P 
```
```{r}
P=1
...
```{r}
data.transformed[,2]=variables_to_transform[,1]^05